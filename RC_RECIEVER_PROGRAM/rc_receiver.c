...
//  Completed
void initial_pwm_pin_init(uint pin){
...
void pwm_set_motor_config(uint pwm_pin, uint16_t channel_value, uint16_t wrap_value){
   
    uint16_t wrap_division = 0x0000;
    uint16_t channel_speed = 0x0000;
    uint16_t unaltered_channel = 0x0000;

    printf("Pwm Set Motor Config Function Active.\n");
    printf("Channel Input Value 0x%04x.\nWrap Input Value: 0x%04x.\n", channel_value, wrap_value);
  
    wrap_division = (wrap_value / 1000);
    printf("Wrap division value: 0x%04x.\n", wrap_division);
    channel_speed = (wrap_division * channel_value) + (.4f * wrap_value);
    unaltered_channel = channel_speed;
    printf("Channel speed value: 0x%04x.\n", channel_speed);
    channel_speed = (channel_value == 0x0000) ? 0x0000 : unaltered_channel;
    printf("For Pin: %i, Wrap: %i, Channel Level: %i.\n\n", pwm_pin, wrap_value, channel_speed);

    pwm_set_wrap(pwm_gpio_to_slice_num(pwm_pin), (wrap_value));
    pwm_set_chan_level(pwm_gpio_to_slice_num(pwm_pin), pwm_gpio_to_channel(pwm_pin), (channel_speed));
...
int drive_motor_wrap_config(uint16_t input_value, uint16_t *wrap_out){

  *wrap_out = 0x0000;

  printf("\nMotor Wrap Function Active.\n\n");
  (input_value > 0) ? printf("Wrap Value to Convert: 0b%04x.\n", input_value) : printf("Input wrap value is NULL.\n");

  (input_value >= 0x00 && input_value <= 0x63) ? (*wrap_out += 0x88B8) : printf("0-99 NULL.\n");
  (input_value >= 0x64 && input_value <= 0xC7) ? (*wrap_out += 0x7530) : printf("100-199 NULL.\n");
  (input_value >= 0xC8 && input_value <= 0x12B) ? (*wrap_out += 0x61A8) : printf("200-299 NULL.\n");
  (input_value >= 0x12C && input_value <= 0x18F) ? (*wrap_out += 0x4E20) : printf("300-399 NULL.\n");
  (input_value >= 0x190 && input_value <= 0x1F3) ? (*wrap_out += 0x3A98) : printf("400-499 NULL.\n");

  (*wrap_out > 0) ? printf("Outputting Wrap: 0b%08x.\n", *wrap_out) : printf("\nMotor Wrap Set NULL.\n");
  return *wrap_out;
  }

int reverse_motor_wrap_config(uint8_t input, uint16_t *wrap_output){

  printf("\nReverse Wrap Function Active.\n");

  previous_reverse_wrap_set_value = 0x0000;

  *wrap_output = 0x0000;

  (input >= 0 && input <= 0x64) ? (*wrap_output += 0x88B8) : previous_reverse_wrap_set_value;
  (input >= 0 && input <= 0x2D) ? (*wrap_output += 0x3A98) : previous_reverse_wrap_set_value;

  return *wrap_output;
}

int motor_drive_function(uint8_t analog_input, uint16_t *motor_speed, uint16_t *reverse_speed){

  printf("\nVertical Analog Input Active.\n");
  printf("Analog input value: %d.\n", analog_input);

  *motor_speed = 0x0000;
  *reverse_speed = 0x0000;
  
  (analog_input >= 0x96 && analog_input < 0xAA) ? (*motor_speed = 0x00) : printf("Motor Driving.\n");

  (analog_input >= 0xB4 && analog_input <= 0xC2) ? (*motor_speed += 0x64) : printf("180-189 NULL.\n");
  (analog_input >= 0xC3 && analog_input <= 0xD1) ? (*motor_speed += 0xC8) : printf("190-199 NULL.\n");
  (analog_input >= 0xD2 && analog_input <= 0xE0) ? (*motor_speed += 0x12C) : printf("200-209 NULL.\n");
  (analog_input >= 0xE1 && analog_input <= 0xEF) ? (*motor_speed += 0x190) : printf("210-219 NULL.\n");
  (analog_input >= 0xF0 && analog_input <= 0xFF) ? (*motor_speed += 0x1F4) : printf("220-225 NULL.\n");

  (analog_input >= 1 && analog_input <= 0x31) ? (*reverse_speed = 0x64) : 0;
  (analog_input >= 0x32 && analog_input <= 0x64) ? (*reverse_speed = 0x12C) : 0;
  
  (*motor_speed > 0) ? printf("\tMotor Speed Set Value: 0b%04x.\n", *motor_speed) : printf("\nMotor Speed Idle\n");

  return *motor_speed | *reverse_speed;
}
