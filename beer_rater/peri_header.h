  ...
  //    Check if -clk = 1 && dt = 0- || -clk = 0 && dt = 1-
    if(((buttons_onoff.rotary_clk_status && !buttons_onoff.rotary_dt_status) || (!buttons_onoff.rotary_clk_status && buttons_onoff.rotary_dt_status))){
      ...
      if(enabled_buttons.rotary_current_value <= enabled_buttons.rotary_maximum_value) {
        ...
        } else if((enabled_buttons.rotary_maximum_value <= 999) && (enabled_buttons.rotary_maximum_value >= 100)){
          enabled_buttons.rotary_current_value += 15;
        } else if((enabled_buttons.rotary_maximum_value <= 99) && (enabled_buttons.rotary_maximum_value >= 0)){
          enabled_buttons.rotary_current_value += 1;
        }else {
...
void rotary_pin_setups(button_types *pin_entry){
    
    gpio_init(pin_entry->rotary_button_clk);
    gpio_set_dir(pin_entry->rotary_button_clk, GPIO_IN);
    gpio_pull_up(pin_entry->rotary_button_clk);

    gpio_init(pin_entry->rotary_button_dt);
    gpio_set_dir(pin_entry->rotary_button_dt, GPIO_IN);
    gpio_pull_up(pin_entry->rotary_button_dt);

    gpio_init(pin_entry->rotary_button_button);
    gpio_set_dir(pin_entry->rotary_button_button, GPIO_IN);
    gpio_pull_up(pin_entry->rotary_button_button);
}

void ultra_sonic_sensor_pin_setup(uint trigger, uint echo) {
  gpio_init(trigger);
...
void serial_register_output(register_pins *config, uint_variables *vars) {
    ...
    if(vars->register_value_zero != NULL){
    ...
    if(vars->register_value_one != NULL){
