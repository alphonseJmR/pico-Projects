...
func_ack ili_initialize(ili_init_var_s *init){
  ...
  printf("\nILI Initialize:\n");
    ...
    status = spi_lpa_command_write((init->data_packet), Power_Control_One, 0x1816);
    f_status = status;
    status = 0;
      sleep_ms(200);
    ...
    status += spi_spa_command_write((init->data_packet), Interface_Mode_Control, 0x00);
    f_status += status;
    status = 0;
      sleep_ms(200);
    ...
    printf("Sleep IN.\n");
    status += spi_spa_command_write((init->data_packet), Sleep_IN, 0);
    f_status += status;
      sleep_ms(200);
