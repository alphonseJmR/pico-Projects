  ...
  if (status == PIN_MNGR_OK)
  {
    ...
    *pins = *user_pins;
    ...
    spi_instance_t spi_instances[3] = {
     instance_pattern[(pins->miso - MISO_MIN) / 4],
     instance_pattern[(pins->mosi - MOSI_MIN) / 4],
...
fn_status_t nrf_driver_send_packet(const void *tx_packet, size_t size) {
   ...
   */
  printf("DEBUG: Pre While.\n");
  uint16_t timeout_max;
  while (((status == SPI_MNGR_OK) && (status_irq == NONE_ASSERTED)) || timeout_max <= 0x0FFF);
  {
     status_irq = check_status_irq(NULL);
     timeout_max++;
  }
  printf("DEBUG: Post While.\n");
